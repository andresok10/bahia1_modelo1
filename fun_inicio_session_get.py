...
def inicio1(id=1):
    ...
    if request.method == 'POST':
        ...
        #cart = session.get('cart', [])  #{'id': 1, 'nombre': 'ok1', 'precio': 10.22}
        actualizar = False
        if idx is not None:
            for x in cart:
                if x["id"] == idx.id:
...
def carritox():
    msg = ""
    cart = session.get('cart', [])
    #request.form.get("id"==id)  # sin product_id
    if request.method == 'POST':
    #if request.method == 'POST' and int(request.form['cantidad']) <= artx.stock:   # 
        cantidadx = int(request.form['cantidad'])
        product_id = int(request.form['product'])
        idx = articulos.query.filter_by(id=product_id).first()
        if idx and cantidadx <= idx.stock:
            #artx = articulos.query.get(id)  # modo antiguo esn esta version ya no vale
            #artx = db.session.get(articulos, id)
            #cart = session.get('cart',[])
            actualizar = False
            for x in cart:
                #print(x)  #{'id': 1, 'img1': 'blusa1.webp', 'nombre': 'ok1', 'precio': 10.22, 'stock': 1}
                #if x['stock'] > idx.stock:
