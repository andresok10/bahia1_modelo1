...
# Inicializar el carrito como variable de sesión
def initialize_cart():
    ...
    if request.method == 'POST':
        ...
        if idx is not None:
            for x in session['cart']:
                ...
                session['cart'].append({'id': idx.id, 'img1': idx.img1, 'nombre': idx.nombre, 'precio': idx.precio, 'stock': cantidadx})
            session['contador'] = len(session['cart'])
...
def carritox():
    ...
    total = sum(item['precio'] * item['stock'] * 1.12 for item in session['cart'])  # Including 12% tax
    #total = sum(item['precio'] * item['stock'] * 0.12 for item in cart)  # Including 12% tax
    session["total"] = total
    return render_template('cart.html', cart=session['cart'], total=total, msg=msg)

######################################################## detalles ########################################
@app_blueprint.route('/detalles/<id>', methods=["get","post"])
def detallesx(id):
    #product = articulos.query.filter_by(id=id).first()
    product = articulos.query.filter_by(id=id).all()
    #product = articulos.query.filter_by(sub_mujer=id).first()
    #product = articulos.query.filter_by(sub_mujer=id).all()
    #product = articulos.query.all()
